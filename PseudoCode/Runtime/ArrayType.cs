...
public class ArrayType : Type
{
    ...
    public override void Assign(Instance to, Instance value)
    {
        if (value.Type.Id != ArrayId)
            throw new InvalidTypeError($"Assigned value should be {Name} OF {((ArrayInstance)to).ElementType}, not {value.Type}", null);
    ...
    {
        if (i2.RealInstance is not ArrayInstance indexInstance) throw new InvalidAccessError("Index access is not an array!", null);
        var arrayInstance = (ArrayInstance)i1.RealInstance;
        if (indexInstance.TotalElements > arrayInstance.Dimensions.Count)
            throw new InvalidAccessError($"Array access indices more than dimensions of array: {indexInstance.TotalElements} > {arrayInstance.Dimensions.Count}", null);
