...
public class IfOperation : Operation
{
    ...
    public override void Operate()
    {
        ...
        var test = Program.FindDefinition(Type.BooleanId).Type.HandledCastFrom(Program.RuntimeStack.Pop());
        if (test.Get<bool>())
    ...
    public override void MetaOperate()
    {
        ...
        if (!Program.FindDefinition(Type.BooleanId).Type.IsConvertableFrom(testType.Type))
    ...
    {
        return
            string.Format(strings.IfOperation_ToString, Indent(depth), TestExpressionScope.ToString(depth),
                Indent(depth), TrueBlock.ToString(depth),
                FalseBlock != null
                    ? string.Format(strings.IfOperation_ToString_Else, Indent(depth), FalseBlock.ToString(depth))
                    : "");
    }
}